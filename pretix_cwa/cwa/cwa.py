...
PUBLIC_KEY_STR = (
    "gwLMzE153tQwAOf2MZoUXXfzWTdlSpfS99iZffmcmxOG9njSK4RTimFOFwDh6t0Tyw8XR01ugDYjtuKwj"
...
class CwaEventDescription(object):
    def __init__(self):
        """Description of the Location, Required, String, max 100
        Characters."""
...
def generatePayload(eventDescription: CwaEventDescription) -> lowlevel.QRCodePayload:
    ...
    payload.locationData.startTimestamp = (
        int(eventDescription.startDateTime.timestamp())
        if eventDescription.startDateTime
        else 0
    )
    payload.locationData.endTimestamp = (
        int(eventDescription.endDateTime.timestamp())
        if eventDescription.endDateTime
        else 0
    )

    payload.crowdNotifierData.version = 1
    payload.crowdNotifierData.publicKey = PUBLIC_KEY
    payload.crowdNotifierData.cryptographicSeed = (
        eventDescription.randomSeed
        if eventDescription.randomSeed is not None
        else secrets.token_bytes(16)
    )

    cwaLocationData = lowlevel.CWALocationData()
    cwaLocationData.version = 1
    cwaLocationData.type = (
        eventDescription.locationType
        if eventDescription.locationType is not None
        else lowlevel.LOCATION_TYPE_UNSPECIFIED
    )
    cwaLocationData.defaultCheckInLengthInMinutes = (
        eventDescription.defaultCheckInLengthInMinutes
        if eventDescription.defaultCheckInLengthInMinutes is not None
        else 0
    )
